...
import { getAbsoluteExpiryCountdown } from "@/lib/quote-expiry";
import { cn } from "@/lib/utils";

type AbsoluteExpiryCountdownLabelProps = {
  expiresAt: string;
  className?: string;
  onExpired?: () => void;
};

/** Live `HH:MM:SS` countdown to an absolute deadline (top-up due, etc.). */
export function AbsoluteExpiryCountdownLabel({
  expiresAt,
  className,
  onExpired,
}: AbsoluteExpiryCountdownLabelProps) {
  ...
  const countdown =
    nowMs == null ? null : getAbsoluteExpiryCountdown(expiresAt, nowMs);
  ...
  return (
    <time
      ...
      title={`Top-up expires ${new Date(countdown.expiresAt).toLocaleString()}.`}
      aria-live="polite"
      aria-atomic="true"
    >
      <span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
