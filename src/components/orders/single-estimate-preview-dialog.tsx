...
type SingleEstimatePreviewRequest = Pick<
  ItemRequest,
  "productName" | "quantity" | "productSize" | "productColor" | "productImageUrl"
>;

export type SingleEstimatePreviewDialogProps = {
  quote: ItemQuote;
  request: SingleEstimatePreviewRequest;
  label?: string;
  dialogTitle?: string;
...
export function SingleEstimatePreviewDialog({
  quote,
  request,
  label = "Preview",
  dialogTitle = "Single estimate summary",
  dialogDescription = "Standalone single-product estimate breakdown and staff notes.",
  staffNotesTitle = "Estimate note (staff)",
}: SingleEstimatePreviewDialogProps) {
      ...
      <DialogContent className="max-h-[min(90vh,42rem)] overflow-y-auto sm:max-w-md">
        ...
        <div className="space-y-4 text-sm">
          ...
          {quote.staffNote?.trim() ?
            <AdminStaffNotesBlock staffNote={quote.staffNote} title={staffNotesTitle} />
          : null}
        </div>
      </DialogContent>
    </Dialog>
