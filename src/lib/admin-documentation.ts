...
const ADMIN_DOCUMENTATION_CONTENT_RAW: DocumentationSection[] = [
  ...
  },
  {
    ...
    quickReference: {
      ...
      bullets: [
        "Search, filter by retailer or publish status, and paginate the catalog table. Each row shows the retailer name (Amazon, Walmart, Temu), product URL, and how many variants are saved. Click headers to sort, including Retailer, Product URL, and Variants. Newest first until you click a header. Double-click a row to edit. Use Add sizes on a color to save extra sizes at the same URL and cost.",
        "Opening the page does not call SerpApi. Scheduled retailer checks (cron) refresh stale listings; changed rows highlight in amber with Check retailer. One SerpApi key is used for customer quote lookups, admin estimates, and Spotlight—several people can search at once if the SerpApi plan’s rate limit allows it.",
        "Paste a product URL and Run SerpApi lookup. Use a full product page: Amazon /dp/ plus 10-character ASIN, Walmart /ip/ plus item ID, Target /p/…/-/A-…, or eBay /itm/—not a truncated, search, or brand store page. If the Amazon ASIN or Walmart item ID is missing, lookup searches that store using the URL title slug. Other retailers use Google Shopping for the same retailer. Stores that bounce the server fetch (geo, cookies, or a redirect loop, for example Bath & Body Works) still fill from Google Shopping when SerpApi finds the listing. HTTP 429 means the plan’s request rate was hit for a moment (one lookup can use several SerpApi calls). HTTP 503 means Google Shopping was briefly down; store variants can still load—run lookup again for retailer comparison. Results use two tabs under the URL: Store variants and Retailer comparison. Search and paginate the variant table (default 10 rows). Walmart size rows often have color-chip thumbs only; those rows use the listing preview photo instead.",
        ...
        "Move a product to another category from the row Actions (variants stay with it).",
    ...
    article: {
      ...
      walkthrough: [
        ...
        "Add products: paste a full retailer product page (Amazon /dp/ASIN, Walmart /ip/ plus item ID, Target /p/…/-/A-…, eBay /itm/, Temu, or SHEIN—not a truncated, search, or /stores/ brand page) and Run SerpApi lookup. If the Amazon ASIN or Walmart item ID is missing, lookup searches that store using the URL title slug. Stores that redirect the server fetch still fill from Google Shopping when possible. Store variants and Retailer comparison are tabs under the URL. HTTP 503 on comparison means Google Shopping was briefly down—run lookup again. Apply fills the form. Save on a variant uses that SKU as the table listing when the product is not saved yet; further Saves add extra SKUs under it. Add product to category saves the current form.",
        "Double-click a record to edit the product name, price, size, color, image, and variants. Shoppers see one compact card per product; double-clicking a card with options opens color swatches and size pills. On a saved color, use Add sizes to paste the remaining sizes (same URL and cost), for example US6-6.5 (CN36-37), US7-7.5 (CN38-39).",
        "Use Move on a row to send the product and its saved variants to a different category tab. Publish status does not change.",
        "Amber Check retailer rows mean a scheduled retailer check found that the live listing no longer matches (price, URL, name, image, or availability). Review with the retailer, then save the edit to clear the warning until the next scheduled check (stale listings older than six hours).",
