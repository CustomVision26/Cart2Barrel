...
export async function compareRetailerPricesAction(
  ...
  try {
        ...
        await verifyShoppingMatchesWithOpenAI(
          {
            ...
            retailer: originalRetailer?.trim() || null,
          },
          candidates,
        )
      : [];
    ...
    if (originalProductUrl?.trim()) {
      const origUrl = originalProductUrl.trim();
      const alreadyListed = verifiedOffers.some(
        (o) => o.productUrl === origUrl,
      );
      if (!alreadyListed) {
        verifiedOffers.unshift({
          id: "original",
          retailer:
            originalRetailer?.trim() ||
            hostnameFromProductUrl(origUrl) ||
            "Original listing",
          title: productName,
          productUrl: origUrl,
          priceUsdCents: originalPriceUsdCents ?? null,
          imageUrl: originalImageUrl?.trim() || null,
        ...
        if (idx >= 0) {
          verifiedOffers[idx] = {
            ...
            matchConfidence: null,
            priceUsdCents:
              verifiedOffers[idx].priceUsdCents ?? originalPriceUsdCents ?? null,
            imageUrl:
              verifiedOffers[idx].imageUrl ?? originalImageUrl?.trim() ?? null,
          };
        }
      }
    }
    ...
    if (verifiedOffers.length === 0) {
      return {
        ok: false,
        message:
          "No matching offers passed verification. Try refining the product name or use Fill details with AI first.",
      };
    }
